...
"Slide title and menu title are blank. If the title is to be left blank, then menu_title needs to be set."
    ...
    def add_plot(self,
        ...
        Parameters
        ----------
        figure_object : matplotlib.figure.Figure, matplotlib.axes.Axes or plotly.graph_objs.Figure
            A Matplotlib `Axes` or `Figure` object, or Plotly `Figure` object. If a Matplotlib object, the object must
            implement the `get_figure()` or `savefig(...)` function. If a Plotly object, then the `.to_html(...)`
...
f"Unknown Matplotlib object passed to add_plot. Please provide a valid matplotlib Figure or Axes object. Received object of type {figure_object.__class__.__name__}."
            ...
            fig = MPLFigure(mpl_fig, row, column, row_span, col_span, matplotlib_params)
        else:
